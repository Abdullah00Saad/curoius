...
def profile():
    ...
    return render_template("profile.html",customers=customers,items=items)
...
def wwu():
    ...
    if request.method == 'POST':
        ...
        db.execute("INSERT INTO workers(fullname,email,address,phone_number,profession,message)VALUES(?,?,?,?,?,?)",name,email,address,phone,profession,message)
        return redirect("/")
    # return render_template("work with us.html")
